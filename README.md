...
一个基于 MyBatis 拦截器的分页与扩展框架，面向 JDK 25 设计，充分利用虚拟线程与 `ScopedValue` 提供高性能、低侵入的分页能力，并抽象出统一的插件链机制，方便在 MyBatis 执行链上做更多扩展（数据隔离、多租户、审计等）。
...
- **分页上下文与结果封装**
  - `Page<E>` 继承自 `ArrayList<E>`，同时包含 `pageNum`、`pageSize`、`total`、`pages`、`orderBy` 等字段
  - `PageContext` 通过 `ScopedValue` 管理当前线程/虚拟线程的分页上下文
- **JDK 25 / 虚拟线程友好**
...
如果需要在非 Spring Boot/MyBatis 模块中直接使用分页能力，可按需引入：
