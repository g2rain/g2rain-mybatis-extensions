...
/**
 * 拦截点枚举，用于标识 MyBatis {@link org.apache.ibatis.executor.Executor} 与
 * {@link org.apache.ibatis.executor.statement.StatementHandler} 等调用类型。
 * <p>
 * 枚举值用于 {@link ExecutorCompositeInterceptor}、{@link StatementHandlerCompositeInterceptor} 中区分拦截逻辑，
 * 并封装各类型调用的默认处理方式。
 * <p>
 * 枚举值说明：
 * <ul>
 ...
 *     <li>{@link #PREPARE} - 语句准备，调用 {@link InvocationContext} 的 proceed 方法。</li>
...
public enum InterceptPoint {
    ...
    /**
     * StatementHandler {@code prepare} 拦截点（SELECT、INSERT、UPDATE、DELETE 语句准备阶段）。
     * <p>
     * 调用 {@link InvocationContext} 的 {@code proceed()} 方法执行原语句准备逻辑。
     */
    PREPARE,
     ...
     * 默认处理方法。
     * <p>
     * 对于未重写 {@code handle} 的枚举值（如 {@link #UPDATE}、{@link #PREPARE}、{@link #UNKNOWN}），
